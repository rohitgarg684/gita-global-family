...
import { type QAItem, type QABlock, type QALang } from "@/data/qa-brahmbodhi";
...
export default function QADetailView({ item }: { item: QAItem }) {
  const hasBilingual = !!item.hi;
  const [lang, setLang] = useState<QALang>("en");

  const t = lang === "hi" && item.hi ? item.hi : item;
  const shareUrl = `/qa-with-brahmbodhi/${encodeURIComponent(item.id)}`;
  const shareDescription = t.preview;
    ...
    <>
      ...
      <section className="section-padding pb-16 md:pb-24">
        <div className="max-w-4xl space-y-6">
          <div className="rounded-2xl border border-cream-dark/40 bg-cream/40 p-5 md:p-6">
            <h2 className="text-base md:text-lg font-bold text-dark-brown mb-3">
              {lang === "hi" ? "इस उत्तर को साझा करें" : "Share this answer"}
            </h2>
            <ShareButtons
              url={shareUrl}
              title={t.question}
